...
class TestMenu:
    ...
    def test_read_empty_menus(self, get_reverse: Callable) -> None:
        response = client.get(
    ...
    def test_create_menu(self, get_reverse: Callable, get_db: Session) -> None:
        menu_data: dict = {
            'title': 'menu1',
            'description': 'description menu1'
        }
        response = client.post(
            url=get_reverse('create_menu'),
            json=menu_data
        )
    ...
    def test_create_menu_with_existing_title(
            self,
            get_reverse: Callable,
            prepare_test_data: tuple[Session, Menu]
    ) -> None:
    ...
    def test_read_menu(self, get_reverse: Callable, get_db: Session, prepare_test_data: tuple[Session, Menu]) -> None:
        session, menu = prepare_test_data
        response = client.get(
            url=get_reverse('read_menu', menu_id=menu.id)
        )
        assert response.status_code == 200
        assert response.json()['id'] == str(menu.id)
        assert response.json()['title'] == menu.title

    def test_read_menu_with_invalid_id(self, get_reverse: Callable) -> None:
        invalid_id: str = 'c11907df-84fe-481c-94fa-fdc9fcab34b0'
        response = client.get(
            url=get_reverse('read_menu', menu_id=invalid_id)
        )
        assert response.status_code == 404
        assert response.json()['detail'] == 'menu not found'

    def test_read_menu_with_not_uuid_id(self, get_reverse: Callable) -> None:
        response = client.get(
            url=get_reverse('read_menu', menu_id='123')
        )
        assert response.status_code == 422
        assert response.json()['detail'][0]['type'] == 'uuid_parsing'

    def test_read_menus(self, get_reverse: Callable, prepare_test_data: tuple[Session, Menu]) -> None:
        session, menu = prepare_test_data
        response = client.get(
            url=get_reverse('read_menus')
        )
    ...
    def test_update_menu(self, get_reverse: Callable, get_db: Session, prepare_test_data: tuple[Session, Menu]) -> None:
        session, menu = prepare_test_data
        menu_data: dict = {
            'title': 'updated menu1',
        ...
        response = client.patch(
            url=get_reverse('update_menu', menu_id=menu.id),
            json=menu_data
        )
        assert response.status_code == 200
        assert response.json()['id'] == str(menu.id)
    ...
    def test_update_menu_with_invalid_id(self, get_reverse: Callable) -> None:
        invalid_id: str = 'c11907df-84fe-481c-94fa-fdc9fcab34b0'
        ...
        response = client.patch(
            url=get_reverse('update_menu', menu_id=invalid_id),
            json=menu_data
        )
        assert response.status_code == 404
        assert response.json()['detail'] == 'menu not found'

    def test_delete_menu(self, get_reverse: Callable, prepare_test_data: tuple[Session, Menu]) -> None:
        session, menu = prepare_test_data
